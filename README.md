...
**Node.js >= 20** · 현재 버전 **2.12.0**

### 2.12 — wiki MR · multi-vault · raw inbox · dashboard

| 기능                                  | CLI                                                                           | MCP                                             |
| ------------------------------------- | ----------------------------------------------------------------------------- | ----------------------------------------------- |
| `vault/raw-inbox/` 드롭 → 자동 ingest | `aio watch-raw`, `aio scan-inbox`                                             | `scan_raw_inbox`                                |
| Multi-vault 레지스트리                | `aio vault list`, `aio vault register` (`vault-list` / `vault-register` 별칭) | `list_vaults`, `register_vault`                 |
| Wiki MR (제안→승인→merge)             | —                                                                             | `propose_wiki_change`, `apply_wiki_proposal`, … |
| 커버리지 대시보드                     | `aio dashboard` (http://127.0.0.1:8920)                                       | `get_dashboard_stats`                           |

**Multi-vault:** `.aio/vaults.yaml`에 등록한 뒤, 활성 vault는 **MCP/CLI 프로세스 시작 시** 결정됩니다 (`AIO_VAULT_NAME` → registry default → `vault/`). `register_vault`는 yaml만 갱신하며, 이름을 바꾼 뒤에는 MCP를 **재시작**해야 합니다.
...
| 5   | `aio ingest --file docs/x.md` or `ingest pipeline` in chat | raw → wiki → lint 한 번에                                     |
...
| 운영       | 툴                                                                         | 동작                                            |
| ---------- | -------------------------------------------------------------------------- | ----------------------------------------------- |
| **Ingest** | `ingest_pipeline` / `ingest_raw` → `ingest_source` / `ingest_source_batch` | raw → wiki(들) → lint. CLI: `aio ingest --file` |
...
`get_wiki_schema` · `ingest_raw` · `ingest_source` · `ingest_source_batch` · `ingest_pipeline` · `update_wiki_page` · `query_wiki` · `file_back` · `lint_wiki`
