...
function inferModulesFromWiki(
  ...
  if (stacks.backend) {
    ...
    for (const page of domainPages) {
      const hit = bcKeywords.find(
        (k) =>
          page.title.toLowerCase().includes(k.toLowerCase()) ||
          page.path.toLowerCase().includes(k.toLowerCase())
      )
...
function buildArchitectureMarkdown(
  ...
  for (const m of modules) {
    ...
    if (m.wiki_citations.length)
      lines.push(`- Citations: ${m.wiki_citations.map((c) => `[[${c}]]`).join(', ')}`)
    lines.push('')
  }
