...
export async function bootstrapHarness(
  ...
  const agentsResult = await writeFile(agentsPath, projectAgentsMd(profile), force, 'shared')
  ...
  if (opts.prune_foreign) {
    const active = detection.target as Exclude<HarnessTarget, 'all'>
    const pruned = await pruneForeignHarnessFiles(projectRoot, active, {
