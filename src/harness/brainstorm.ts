...
function matchDomainPatterns(topic: string): DomainPatternRule[] {
  ...
  return DOMAIN_PATTERNS.filter((r) => r.keywords.some((k) => lower.includes(k.toLowerCase())))
}
...
function buildAgentInstructions(
  ...
  return [
    ...
    '',
    `Options on the table (${options.length}): ${options
      .slice(0, 8)
      .map((o) => `[${o.focus}] ${o.name}`)
      .join('; ')}`,
  ].join('\n')
}
  ...
  const recommendation = {
    ...
    rationale: [
      ...
      wikiCitations.length
        ? `Wiki: ${wikiCitations.slice(0, 3).join(', ')}`
        : 'Expand wiki for richer brainstorm.',
    ].join(' '),
