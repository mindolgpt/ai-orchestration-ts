...
export function detectHarnessTargetFromEnv(): HarnessTargetDetection | null {
  ...
  if (process.env.CURSOR_PROJECT_DIR || process.env.CURSOR_TRACE_ID || process.env.CURSOR_AGENT) {
...
export function resolveTargets(targets?: HarnessTarget[], _projectRoot?: string): HarnessTarget[] {
...
/** Remove harness files belonging to non-active AI tools */
export async function pruneForeignHarnessFiles(
  projectRoot: string,
  activeTarget: Exclude<HarnessTarget, 'all'>,
  opts?: { dry_run?: boolean }
): Promise<
  Array<{ rel: string; target: HarnessTarget; action: 'deleted' | 'would_delete' | 'missing' }>
> {
  const foreign = await findForeignHarnessFiles(projectRoot, activeTarget)
  const results: Array<{
    rel: string
    target: HarnessTarget
    action: 'deleted' | 'would_delete' | 'missing'
  }> = []

  for (const f of foreign) {
    const abs = path.join(projectRoot, f.rel)
    if (!(await pathExists(abs))) {
      results.push({ rel: f.rel, target: f.target, action: 'missing' })
      continue
    }
    if (opts?.dry_run) {
      results.push({ rel: f.rel, target: f.target, action: 'would_delete' })
      continue
    }
    await fs.unlink(abs)
    results.push({ rel: f.rel, target: f.target, action: 'deleted' })
  }
  return results
}
