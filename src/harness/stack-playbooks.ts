...
export function detectStacksFromText(text: string): {
  frontend?: string
  backend?: string
  mobile?: string
  api?: string
  ...
  for (const p of STACK_PLAYBOOKS) {
    if (
      p.aliases.some((a) => text.toLowerCase().includes(a.toLowerCase())) ||
