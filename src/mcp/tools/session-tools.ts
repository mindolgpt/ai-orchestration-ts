...
export async function spawnSession(
  ...
  proc.on("error", (err) => {
    ...
    getEventLog().emit("session.error", { session_id: sessionId, error: err.message }, "error");
  });
  ...
  proc.on("close", (code) => {
    ...
    getEventLog().emit("session.closed", {
...
export async function waitForSession(
  ...
  if (session) {
    ...
    await getEventLog().emit("session.timeout", { session_id: sessionId, timeout_ms: timeoutMs }, "warn");
  }
...
export function synthesizeResults(
  ...
  const sessionSummaries = sessionList.map((s) => {
    ...
    return {
      ...
      task: s.task,
      summary:
        (last?.payload?.summary as string | undefined) ||
        s.stdout.trim().slice(0, 400) ||
        null,
...
export function registerSessionTools(
  ...
  server.registerTool(
    ...
    async (args) => {
      ...
      if (s && (args.status === "completed" || args.status === "failed")) {
        s.status = args.status as ChildSession["status"];
      }
  ...
  server.registerTool(
    ...
    {
      description:
        "Queue an instruction for a session (pending_messages). Not a live IPC channel.",
