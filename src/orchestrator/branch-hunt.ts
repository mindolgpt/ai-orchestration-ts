...
const ISSUE_PATTERNS: Array<{ re: RegExp; severity: Issue["severity"]; label: string; rg: string }> = [
  ...
  { re: /catch\s*\(\s*\w*\s*\)\s*\{\s*\}/g, severity: "high", label: "empty-catch", rg: "catch\\s*\\([^)]*\\)\\s*\\{\\s*\\}" },
  { re: /\beval\s*\(/g, severity: "critical", label: "eval", rg: "\\beval\\s*\\(" },
];

const CODE_EXT = new Set([".ts", ".tsx", ".js", ".jsx", ".mjs", ".cjs", ".py", ".go", ".java", ".kt"]);
...
export class BranchHunt {
  ...
  async spawnFixes(
    ...
    for (const issue of pending) {
      ...
      if (spawnResult.error) {
        issue.resolution = String(spawnResult.error);
        continue;
      }
  ...
  async collectResults(sessions?: Map<string, ChildSession>): Promise<
