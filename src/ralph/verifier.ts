...
function parseStepsEnv(): VerifyStep[] | undefined {
  ...
  return raw
    ...
    .filter((s): s is VerifyStep =>
      s === "build" || s === "lint" || s === "test" || s === "custom"
    );
}
...
export class Verifier {
  ...
  private async runNpm(script: string): Promise<{ ok: boolean; detail: string; skipped?: boolean }> {
