...
describe('runDoctor vault registry', () => {
  test('uses registry default vault path', async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), 'aio-doc-reg-'))
    const custom = path.join(root, 'custom-vault')
    const vault = new ObsidianVault(custom)
    await vault.initialize()
    await vault.writeNote('wiki/a', '# A', ['wiki'])
    await vault.writeNote('wiki/b', '# B', ['wiki'])
    await vault.writeNote('wiki/c', '# C', ['wiki'])
    await fs.mkdir(path.join(custom, '.index'), { recursive: true })
    await fs.writeFile(path.join(custom, '.index', 'meta.json'), '{}', 'utf-8')
    await fs.writeFile(path.join(custom, '.index', 'index.faiss'), '', 'utf-8')
    await registerVault({ name: 'main', path: 'custom-vault', default: true }, root)

    const prevPath = process.env.AIO_VAULT_PATH
    const prevName = process.env.AIO_VAULT_NAME
    delete process.env.AIO_VAULT_PATH
    delete process.env.OBSIDIAN_VAULT_PATH
    delete process.env.AIO_VAULT_NAME

    const report = await runDoctor({ projectRoot: root, skipEmbedTest: true })
    expect(report.vault_root).toBe(custom)

    if (prevPath) process.env.AIO_VAULT_PATH = prevPath
    if (prevName) process.env.AIO_VAULT_NAME = prevName
  })
})
