...
describe('ObsidianVault', () => {
  ...
  test('resolvePath rejects path traversal', async () => {
    await vault.initialize()
    expect(() => vault.resolvePath('wiki/../../outside')).toThrow(/escapes|Invalid vault path/)
    await expect(vault.writeNote('wiki/../../outside', 'x')).rejects.toThrow(
      /escapes|Invalid vault path/
    )
  })
